#!/usr/bin/env python3
"""
ROS2 Diagnostics TUI - A comprehensive diagnostic tool for ROS2 robot monitoring.
...
Supports multiple robot configurations via YAML config files.
...
from sensor_msgs.msg import Image, Imu, JointState, Joy, LaserScan
...
def get_ros_ws_src_path() -> str:
    ...
    2. Returning empty string if walking fails
    """
    ...
    # Return empty string (config file checks will fail gracefully)
    return ""
...
def discover_robot_configs() -> List[str]:
    ...
    """
    try:
        pkg_share = get_package_share_directory("ros2_diag_tui")
...
def load_config(robot_name: str = "turtlebot3") -> Dict[str, Any]:
...
class DiagTUINode(Node):
    """ROS2 node for robot diagnostics monitoring."""

    def __init__(self, robot_name: str = "turtlebot3"):
        super().__init__("ros2_diag_tui")
        ...
        self.get_logger().info("ROS2 diagnostics TUI node started")
    ...
    def _store_message_summary(self, topic_name: str, msg):
        ...
        if msg_type == "Imu":
            ...
            summary["encoding"] = msg.encoding
            summary["width"] = msg.width
            summary["height"] = msg.height
            summary["step"] = msg.step
...
class DiagTUI:
    """Curses-based TUI for ROS2 diagnostics."""
    ...
    def __init__(self, node: DiagTUINode):
        self.node = node
        self.running = True
        self.stdscr = None
    ...
    def export_snapshot(self):
        ...
        try:
            ...
            lines = []
            lines.append("=" * 70)
            lines.append(f" ROS2 DIAG TUI [{self.node.robot_name}] - Snapshot")
        ...
        while self.running:
            try:
                ...
                if self.show_lidar:
                    ...
                    # Title bar
                    title = f" ROS2 DIAG TUI [{self.node.robot_name}] - q:quit r:robot d:domain e:edit n:net l:lidar m:map v:log p:preview a:alerts s:save h:help "
                    self.safe_addstr(
    ...
    def run_simple(self):
        ...
        print("ROS2 DIAG TUI - Simple Mode (no TTY detected)")
        print("=" * 70)
        ...
        while self.running:
            try:
                ...
                print("=" * 70)
                print(f" ROS2 DIAG TUI [{self.node.robot_name}] - Simple Mode")
                print("=" * 70)
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="ROS2 Diagnostics TUI")
    parser.add_argument(
        "-d",
        "--domain-id",
        type=int,
    ...
    parser.add_argument(
        ...
        "If not specified, uses the first available config.",
    )
    parser.add_argument(
        "--preflight",
        action="store_true",
...
def run_preflight(node: DiagTUINode, timeout: int):
...
def main(args=None):
    ...
    if robot_name is None:
        available = discover_robot_configs()
        robot_name = available[0] if available else "turtlebot3"

    if cli_args.preflight:
        # Preflight mode - run readiness check and exit
        rclpy.init(args=args)
        node = DiagTUINode(robot_name=robot_name)
